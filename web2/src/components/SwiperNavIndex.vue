...
export default {
  ...
  data() {
    return {
      ...
      startAtBoot: 0,
      tabs: [{ name: "Video", comp: "ByVideo2", label: "video" }],
    };
  },
  props: ["typeId"],
  ...
  methods: {
    getConfig() {
      $.getJSON("/api/cmd?cmd=config").then((r) => {
        console.log(r);
        Object.assign(this.config, r);
        let all = Object.keys(r);
        this.keys.length = 0;
        for (let i of all) {
          if (typeof r[i] == "boolean") this.keys.push(i);
        }
      });
    },
    saveConfig() {
      $.getJSON(
        "/api/cmd?cmd=config&val=" +
