...
public class GsyTvVideoView extends MyExo2ListPlayerView implements Player.Listener {
    ...
    public void onError(int what, int extra) {
        ...
        if (setFallbackVideoURI()) return;
        if (mPlayPosition < mUriList.size() - 1) {
            playNext();
        } else PlayerController.getInstance().nextFolderFile();
    }
    ...
    public boolean setFallbackVideoURI() {

        if (isfallback) return false;
        String url = PlayerController.getInstance().getFallBackUrl();
        ...
        if (PlayerController.getInstance().configStore.usingBiliPlayer
        ) {
            PlayerFactory.setPlayManager(IjkPlayerManager.class);
        } else {
        ...
        VFile res = PlayerController.getInstance().getFile();
        urls.add(new GSYVideoModel(url, res.getName() + "(" + res.getPage() + ")"));
    ...
    public void setVideoURI(VFile res, int fi) {
        ...
        mSubtitleView.setVisibility(PlayerController.getInstance().configStore.subTitleActive ? VISIBLE : GONE);
    ...
    private void setUpUrls(VFile res, int fi) {
        ...
        List<GSYVideoModel> urls = new ArrayList<>();
        ConfigStore configStore = PlayerController.getInstance().configStore;
        String baseurl = SSLSocketClient.ServerManager.getServerHttpAddress();
        ...
        for (int i = curIndex; i < files.length; i++) {
            ...
            if (file.getCc() != null) {
                url = httpUrl;
                try {
                    if (configStore.subTitleActive) {
                        url = httpUrl + "&subtitle=" + URLEncoder.encode(file.getCc(), "UTF-8");

                        if (configStore.subTitleActiveTran) {
                            url = httpUrl + "&subtitle=" + URLEncoder.encode(
                                    baseurl + "/api/subtitle?id=" + file.getId() + "&url=" +
                                            URLEncoder.encode(file.getCc(), "UTF-8"), "UTF-8");
                        }
                    }
                ...
                url = httpUrl;
            } else if (file.getdLink() != null) {
                url = App.getUrl(file.getdLink());
            } else {
                url = httpUrl;
            }
